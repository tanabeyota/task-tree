...
export default function TaskNode({ id, data, selected }: NodeProps<TaskNodeData>) {
  ...
  useEffect(() => {
    const unsubscribeLocks = subscribeToLocks(id, (uid) => setLockedBy(uid));
    const unsubscribeConn = subscribeToConnection((status) => setIsOnline(status));
    return () => {
      unsubscribeLocks();
      unsubscribeConn();
    };
  ...
  const isLockedByOther = (lockedBy !== null && lockedBy !== auth.currentUser?.uid) || !isOnline;
  ...
  // デバウンス中のタブ閉じによるデータロスト対策 (Phase 8) & スリープ対策
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (saveTimeoutRef.current && editor) {
         clearTimeout(saveTimeoutRef.current);
         saveTimeoutRef.current = null;
         updateFirestoreNode(id, { html: editor.getHTML() });
      }
    };
    const handleVisibilityChange = () => {
      if (document.hidden && editor && isEditing) {
        editor.commands.blur(); // 強制的にフォーカスを外し、ロック解除と保存を走らせる
      }
    };
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    // TiptapとZundoの競合分離 (Phase 8): Tiptap内でCtrl+Zされた場合、イベント伝播をストップ
    ...
    >
      ...
      <div className="task-node-content">
        <div ref={nodeInputRef} className="node-input nodrag" onKeyDownCapture={handleKeyDown}>
           ...
           {isLockedByOther && <div className="lock-indicator" style={{ position: 'absolute', top: -20, right: 0, fontSize: '0.75rem', color: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', padding: '2px 6px', borderRadius: '4px' }}>{(!isOnline) ? '🔴 Offline' : '🔒 Editing'}</div>}
        </div>
